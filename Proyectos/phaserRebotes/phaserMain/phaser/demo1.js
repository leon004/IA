...
function create() {
    ...
    // Añadir controles manuales
    var up = juego.input.keyboard.addKey(Phaser.Keyboard.W);
    var left = juego.input.keyboard.addKey(Phaser.Keyboard.A);
    var down = juego.input.keyboard.addKey(Phaser.Keyboard.S);
    var right = juego.input.keyboard.addKey(Phaser.Keyboard.D);

    up.onDown.add(moveUp, this);
    left.onDown.add(moveLeft, this);
    down.onDown.add(moveDown, this);
...
function update() {
    ...
    if (modoAuto == false) {
        ...
        if (avanza.isDown && jugador.body.onFloor()) {
            correr();
        }

        if (jugador.body.onFloor() && jugador.position.x >= 200) {
            correrAtras();
        }

        if (!avanza.isDown && jugador.body.onFloor() && jugador.position.x == 50) {
            detenerse();
        }
...
function render() {
    // Agrega cualquier lógica de renderizado aquí
}
